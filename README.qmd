...
In the code below, we filter flows to keep only those between 1 and 20 kilometers.
...
there is no out-of-the-box support for adding weights in other clustering algorithms such
as `hdbscan` or `optics`, so we will use {[dbscan](https://github.com/mhahsler/dbscan)} 
...
Adjust `eps` and `minPts` to control cluster tightness and minimum cluster size.
Cluster composition is determined by these DBSCAN parameters.
