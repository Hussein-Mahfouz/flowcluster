...
After clustering, you may want to aggregate the flows in each cluster into a single line representing the average flow direction and magnitude. This can be useful for visualisation or further analysis. The package has a function to do this, which creates a linestring for each cluster based on the average start and end coordinates of the flows in that cluster.
...
#| echo: false
# save the tmap output
tmap_save(tm = flows_aggregated_map, filename = "man/figures/cluster_aggregated_results.png", width = 8, dpi = 1080, asp = 0)
# load the saved map
```
